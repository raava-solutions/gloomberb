...
1. **Default posture = full tools, confined to a per-thread workspace.** By default a thread's agent can research, run bash, and write scripts/files freely — but confined to its per-thread isolated workspace (`ensureIsolatedThreadWorkspace` from Phase 1a). This is "not crippled" (bash + scripts + write all work) without letting the agent mutate or exfiltrate the real machine. `rm -rf` is possible *inside the workspace* and that's acceptable — the workspace is disposable. **Confinement must be OS-enforced, not advisory (see Phase 0.3b); any provider that can't actually enforce it defaults to read-only, never silent-YOLO.** **AS SHIPPED (security-reviewed 2026-07-20 — corrected known-limitation):** confined guarantees **no host writes outside the workspace** and **no network egress** for all three providers (codex writable roots pinned via `exclude_tmpdir_env_var`/`exclude_slash_tmp`/`network_access=false`; pi has no network-capable tool). **Claude confined is additionally a full READ jail** (seatbelt `denyRead`/`allowRead`). **codex and pi confined can still READ host files** (e.g. `~/.gloomberb` config, broker instance configs, SSH keys) **into the agent's model context** — which transits to the LLM provider and can land in the local transcript. That is a *disclosure* limitation, NOT a remote-exfil hole (no write-out, no network in confined), and is surfaced to the user via the per-provider confined footer caveat. The earlier claim that confined "cannot reach portfolio/account data" holds for claude but is false for codex/pi reads — this is the accurate posture of record.
